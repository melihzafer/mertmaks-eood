...
export function StoreMap({ stores, height = '500px' }: StoreMapProps) {
  ...
  if (!mounted) {
    return (
      <div className='w-full rounded-xl bg-gray-100 flex items-center justify-center' style={{ height }}>
        <div className='text-gray-400'>Loading map...</div>
      </div>
    );
  }
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid md:grid-cols-3 gap-4'>
        {stores.map((store) => (
          <div key={store.id} className='bg-white rounded-lg p-4 border shadow-sm hover:shadow-md transition-shadow'>
